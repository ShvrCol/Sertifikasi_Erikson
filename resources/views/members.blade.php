    ...
    <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <h2 class="mt-5 mb-5 text-center text-2xl font-bold text-gray-900 dark:text-white">Members</h2>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @foreach ( $dashAdminMembers as $hasilAdminMembers)
                            <tr class="border-b dark:border-gray-700 hover:bg-emerald-100">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$hasilAdminMembers->nama_member}}</th>
                                <td class="px-4 py-3">{{$hasilAdminMembers->email}}</td>
                                <td class="px-4 py-3">{{$hasilAdminMembers->alamat}}</td>
                                <td class="px-4 py-3">{{$hasilAdminMembers->nomor_tlp}}</td>
                                <td class="px-4 py-3">
                                    <form action="/members" method="POST" id="deleteMemberForm">
                                        @csrf
                                        <input type="hidden" name="id_member" value="{{$hasilAdminMembers->id_member}}">
                                        <button type="button" onclick="confirmDelete()" class="border bg-red-500 px-5 py-1 rounded-md text-white font-medium hover:bg-red-700" data-bs-toggle="modal" data-bs-target="#modalDelete" value="Delete">Delete</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
